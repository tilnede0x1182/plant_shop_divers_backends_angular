...
static void fill_plant(Plant *p, PGresult *r, int row) {
    ...
    // La description n'est pas récupérée dans la requête de base, mais on la garde dans le modèle
...
int plant_repo_add(PGconn *c, const Plant *p) {
    char pr[12], st[12];
    sprintf(pr, "%d", p->price);
    sprintf(st, "%d", p->stock);
...
int plant_repo_find(PGconn *c, int id, Plant *p) {
    ...
    sprintf(sid, "%d", id);
    const char *v[1] = {sid};
    PGresult *r = PQexecParams(c, "SELECT id,name,price,stock FROM plants WHERE id=$1", 1, NULL, v, NULL, NULL, 0);
...
void plant_repo_patch(PGconn *c, int id, cJSON *j) {
    ...
    if (price_json && cJSON_IsNumber(price_json)) {
        char price_str[12];
        sprintf(price_str, "%d", price_json->valueint);
        const char *p[2] = {price_str, sid};
        PQclear(PQexecParams(c, "UPDATE plants SET price=$1 WHERE id=$2", 2, NULL, p, NULL, NULL, 0));
    }
    ...
    if (stock_json && cJSON_IsNumber(stock_json)) {
        ...
        PQclear(PQexecParams(c, "UPDATE plants SET stock=$1 WHERE id=$2", 2, NULL, p, NULL, NULL, 0));
    }
}
