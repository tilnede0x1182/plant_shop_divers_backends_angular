...
async fn login(email: &str, password: &str, who: &str) -> Result<(), Box<dyn Error>> {
    ...
    hit("POST", "/auth/login", 201, Some(body), who).await?;
    Ok(())
}
...
async fn register_user(name: &str, email: &str, password: &str, who: &str) -> Result<(), Box<dyn Error>> {
    ...
    hit("POST", "/auth/register", 201, Some(body), who).await?;
    Ok(())
}

async fn find_user_id_by_email(who: &str, email: &str) -> Result<u32, Box<dyn Error>> {
    let users = hit("GET", "/users", 200, None, who).await?;
...
async fn test_plants(who: &str) -> Result<(), Box<dyn Error>> {
    ...
    let plant_id = hit("POST", "/admin/plants", 201, Some(plant_data), who).await?.get("id").unwrap().as_u64().unwrap() as u32;

    assert_eq(&hit("GET", &format!("/plants/{}", plant_id), 200, None, who).await?, "name", "Test Plant");

    let updated_plant_data = json!({ "price": 15 });
    hit("PATCH", &format!("/admin/plants/{}", plant_id), 200, Some(updated_plant_data), who).await?;

    assert_eq(&hit("GET", &format!("/plants/{}", plant_id), 200, None, who).await?, "price", 15);

    hit("DELETE", &format!("/admin/plants/{}", plant_id), 200, None, who).await?;
...
async fn test_users(who: &str) -> Result<(), Box<dyn Error>> {
    ...
    let user_id = hit("POST", "/users", 201, Some(user_data), who).await?.get("id").unwrap().as_u64().unwrap() as u32;

    let updated_user_data = json!({ "name": "Tester Update" });
    hit("PATCH", &format!("/users/{}", user_id), 200, Some(updated_user_data), who).await?;

    assert_eq(&hit("GET", &format!("/users/{}", user_id), 200, None, who).await?, "name", "Tester Update");

    hit("DELETE", &format!("/users/{}", user_id), 200, None, who).await?;
...
async fn test_orders(admin_who: &str, user_who: &str) -> Result<(), Box<dyn Error>> {
    ...
    let plant_id = hit("POST", "/admin/plants", 201, Some(plant_data), admin_who).await?.get("id").unwrap().as_u64().unwrap() as u32;

    let order_payload = json!({ "items": [{ "plantId": plant_id, "quantity": 2 }] });
    let order_id = hit("POST", "/orders", 201, Some(order_payload), user_who).await?.get("id").unwrap().as_u64().unwrap() as u32;

    let updated_order_data = json!({ "status": "shipped" });
    hit("PATCH", &format!("/orders/{}", order_id), 200, Some(updated_order_data), admin_who).await?;

    let commandes = hit("GET", "/orders", 200, None, user_who).await?;
    ...
    hit("DELETE", &format!("/orders/{}", order_id), 200, None, admin_who).await?;
    hit("DELETE", &format!("/admin/plants/{}", plant_id), 200, None, admin_who).await?;
...
async fn test_user_profile(admin_who: &str, user_who: &str, user_email: &str) -> Result<(), Box<dyn Error>> {
    ...
    let user_id = find_user_id_by_email(admin_who, user_email).await?;

    assert_eq(&hit("GET", &format!("/users/{}", user_id), 200, None, user_who).await?, "id", user_id);

    let updated_user_data = json!({ "name": "Utilisateur_de_test" });
    hit("PATCH", &format!("/users/{}", user_id), 200, Some(updated_user_data), user_who).await?;

    assert_eq(&hit("GET", &format!("/users/{}", user_id), 200, None, user_who).await?, "name", "Utilisateur_de_test");

    let elevation_data = json!({ "admin": true });
    hit("PATCH", &format!("/users/{}", user_id), 200, Some(elevation_data), user_who).await?;

    let profil = hit("GET", &format!("/users/{}", user_id), 200, None, admin_who).await?;
...
async fn test_auth_roles(admin_who: &str, user_who: &str) -> Result<(), Box<dyn Error>> {
    ...
    hit("POST", "/admin/plants", 403, Some(bad_plant_data), user_who).await?;

    let good_plant_data = json!({ "name": "Good", "price": 1, "stock": 1 });
    let plant_id = hit("POST", "/admin/plants", 201, Some(good_plant_data), admin_who).await?.get("id").unwrap().as_u64().unwrap() as u32;
    hit("DELETE", &format!("/admin/plants/{}", plant_id), 200, None, admin_who).await?;

    hit("GET", "/users", 403, None, user_who).await?;
...
async fn test_admin_plants(who: &str) -> Result<(), Box<dyn Error>> {
    ...
    let plantes = hit("GET", "/admin/plants", 200, None, who).await?;
...
async fn test_admin_users(who: &str) -> Result<(), Box<dyn Error>> {
    ...
    let utilisateurs = hit("GET", "/admin/users", 200, None, who).await?;
    ...
    hit("PATCH", &format!("/admin/users/{}", u.get("id").unwrap().as_u64().unwrap() as u32), 200, Some(updated_user_data), who).await?;

    assert_eq(&hit("GET", &format!("/users/{}", u.get("id").unwrap().as_u64().unwrap() as u32), 200, None, who).await?, "name", nom_modifie);
...
async fn test_auth_me(who: &str) -> Result<(), Box<dyn Error>> {
    println!("\n📌 TEST MODULE: AUTH /me");
    let me = hit("GET", "/auth/me", 200, None, who).await?;
...
async fn main() -> Result<(), Box<dyn Error>> {
    ...
    login(&config.admin_email, &config.admin_password, "admin").await?;
    let user_email = "utilisateur_de_test@example.com";
    register_user("User", user_email, "pass123", "user").await?;

    test_plants("admin").await?;
    test_users("admin").await?;
    test_orders("admin", "user").await?;
    test_user_profile("admin", "user", user_email).await?;
    test_auth_roles("admin", "user").await?;
    test_admin_plants("admin").await?;
    test_admin_users("admin").await?;
    test_auth_me("user").await?;
